...
contract Marketplace {
    ...
    modifier hasSufficientFunds(uint256 price) {
    require(lorToken.balanceOf(msg.sender) >= price, "Not enough LOR balance");
    _;
}

    modifier nftExists(uint256 id) {
    require(id > 0 && id <= nftCount, "NFT does not exist");
    _;
}
    ...
    function buyNFT(uint256 id) external nftExists(id) hasSufficientFunds(nfts[id].price) {
